...
public class Customer_ implements Serializable {
	...
	@Id
	@SequenceGenerator(name="CUSTOMER_ID_GENERATOR_", sequenceName="SEQ_CUSTOMER", schema="MAPSHOP", allocationSize=1, initialValue=1)
	@GeneratedValue(strategy=GenerationType.SEQUENCE, generator="CUSTOMER_ID_GENERATOR_")
	private Long id;
    ...
    public boolean equals(
        ...
        if (
            !(obj instanceof Customer_)
        ) {
            return false;
        }
        Customer_ other = (Customer_) obj;
        if (
            id == null
        ) {
            if (
    ...
    public String toString() {
        return "Customer [id=" + id + ", email=" + email
                + ", password=" + password + ", offers="
                + offers + ", purchases=" + purchases + "]";
    }
